...
import { updateMountedGroup, type MountedGroups } from '../mutable-state/groups';
	...
	hitRegions.forEach((current) => {
		...
		if (pointerDownAtPoint) {
			if (orientation === 'horizontal') {
				deltaAsPercentage = ((event.clientX - pointerDownAtPoint.x) / groupSize) * 100;
			} else {
				deltaAsPercentage = ((event.clientY - pointerDownAtPoint.y) / groupSize) * 100;
			}
		} else {
			if (orientation === 'horizontal') {
			...
			if (layoutsEqual(nextLayout, prevLayout)) {
				if (deltaAsPercentage !== 0 && !disableCursor) {
					...
					switch (orientation) {
						case 'horizontal': {
							nextCursorFlags |=
								deltaAsPercentage < 0 ? CURSOR_FLAG_HORIZONTAL_MIN : CURSOR_FLAG_HORIZONTAL_MAX;
							break;
						}
						case 'vertical': {
							nextCursorFlags |=
								deltaAsPercentage < 0 ? CURSOR_FLAG_VERTICAL_MIN : CURSOR_FLAG_VERTICAL_MAX;
							break;
						}
					}
				}
