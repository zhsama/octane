...
export function getImperativePanelMethods({
	...
	const find = () => {
		...
		for (const [
			group,
			{ defaultLayoutDeferred, derivedPanelConstraints, layout, groupSize, separatorToPanels },
		] of mountedGroups) {
			if (group.id === groupId) {
				return {
		...
		const allPreviousCollapsed =
			...
			(isFirstPanel ||
				panels.slice(0, index).every((_panel, panelIndex) => {
					...
					return pc?.collapsible && layoutNumbersEqual(pc.collapsedSize, prevLayout[pc.panelId]);
				}));
	...
	return {
		...
		getSize: () => {
			...
			const inPixels =
				group.orientation === 'horizontal' ? element.offsetWidth : element.offsetHeight;
