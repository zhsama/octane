...
export class EventEmitter<Events extends EventMap> {
	...
	addListener<Type extends keyof Events>(type: Type, listener: EventListener<Events[Type]>) {
	...
	removeListener<Type extends keyof Events>(type: Type, listener: EventListener<Events[Type]>) {
		const listeners = this.#listenerMap[type];
		if (listeners !== undefined) {
