...
const prettierOptions = (await resolveConfig(join(sourceRoot, 'index.tsrx.d.ts'))) ?? {};

if (outputFlagIndex !== -1 && process.argv[outputFlagIndex + 1] === undefined) {
	throw new Error('Missing directory after --out-dir.');
}
if (checking && outputRoot !== null) throw new Error('--check and --out-dir cannot be combined.');
...
async function formatDeclaration(source) {
...
const temporaryRoot = mkdtempSync(join(tmpdir(), 'octane-react-pdf-types-'));
...
try {
	...
	if (result.emitSkipped) throw new Error('TypeScript skipped React-PDF declaration generation.');
	...
	if (outputRoot !== null) {
		...
		console.log(`React-PDF package declarations emitted (${sources.length} modules).`);
	} else {
		const changed = [];
		...
		if (checking && changed.length) {
			throw new Error(`Stale React-PDF TSRX declarations:\n${changed.join('\n')}`);
		}
		console.log(`React-PDF TSRX declarations are current (${components.length} components).`);
	}
} finally {
