...
def select(v):
    ...
    selected = re.sub(r'"packageManager": "pnpm@[^\"]+"', '"packageManager": "pnpm@' + VERSIONS[v] + '"', ORIGINAL)
    assert json.loads(selected)['packageManager'] == 'pnpm@' + VERSIONS[v]
    if manifest.read_text() != selected:
        manifest.write_text(selected)
    env = ENV.copy()
    env['PATH'] = str(BASE / 'tools' / ('pnpm' + v) / 'node_modules/.bin') + os.pathsep + env['PATH']
...
def measure(v, scenario, iteration, args):
    ...
    cmd = [str(BASE / 'tools' / ('pnpm' + v) / 'node_modules/.bin/pnpm'), *args]
    if args[0] == 'install':
...
if __name__ == '__main__':
    for v, version in VERSIONS.items():
        actual = subprocess.check_output([str(BASE/'tools'/('pnpm'+v)/'node_modules/.bin/pnpm'),'--version'],cwd=ROOT,env=select(v),text=True).strip()
        assert actual==version, (actual,version)
    baseline = '09707df629c170a878e1f238921e09b86a9662a4'
    baseline_lock = subprocess.check_output(['git', 'show', baseline + ':pnpm-lock.yaml'], cwd=ROOT)
    combined_lock = (ROOT / 'pnpm-lock.yaml').read_bytes()
    assert combined_lock.endswith(baseline_lock), 'Application lock document must be byte-identical to baseline'
    lock_comparison = {'base':baseline,'application_lock_unchanged':True,'tool_document_bytes':len(combined_lock)-len(baseline_lock),'original_sha256':hashlib.sha256(baseline_lock).hexdigest(),'combined_sha256':LOCK}
    (BASE/'lockfile-comparison.json').write_text(json.dumps(lock_comparison,indent=2)+'\n')
